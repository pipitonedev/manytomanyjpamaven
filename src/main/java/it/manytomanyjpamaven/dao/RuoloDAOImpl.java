...
public class RuoloDAOImpl implements RuoloDAO {
	...
	public List<Ruolo> list() throws Exception {
		return entityManager.createQuery("from Utente", Ruolo.class).getResultList();
	}
	...
	public void delete(Ruolo ruoloInstance) throws Exception {
		...
		TypedQuery<Utente> query = entityManager
				.createQuery("select u from Utente u join fetch u.ruoli r where r.codice like?1", Utente.class);
		if (query.setParameter(1, ruoloInstance.getCodice()).getResultList().size() != 0) {
			throw new Exception("Impossibile eliminare il campo!");
		}
